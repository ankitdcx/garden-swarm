...
1. **Nine v15.6 GSL gap closures** — tracked structurally in private `garden-main/design_deltas/v15.6/DELTASET-2026-09-13-GSL-GAP-CLOSURE.json`: bootstrap trust-root ceremony; shared-state authority provenance; deterministic let/query semantics; causal-level typing; R0 SanityGate separation; rights-view ordering; provenance redaction-vs-absence typing; and catalogue closure for `AuthorityDecl` / `EffectDecl`.
...
- **WP-009H shared DesignEpoch convergence vectors** — important implementation/conformance hardening; already merged in `garden-main`. It becomes a source-design delta only if convergence testing exposes a missing semantic requirement.
