...
CANDIDATE=Path('review-inputs/v159-postfreeze/SYNTHESIZED_POSTFREEZE_CANDIDATE_R2.txt')
EVIDENCE=Path('review-inputs/v159-postfreeze/FINAL_BOARD_EVIDENCE_INDEX.json')
OUT=Path('review-results/v159-postfreeze-final-challenge')
DAILY_CEILING=10.0
...
def call(model,prompt,key):
 ...
 cmd=['curl','-sS','--connect-timeout','10','--max-time','360',OPENROUTER,'-X','POST',
      ...
      '-H','X-Title: Garden v15.9 Final Adversarial Challenge',
      '--data-binary','@-','-w','\\n%{http_code}']
...
def main():
 ...
 prompt=f'''FINAL ADVERSARIAL CHALLENGE — GARDEN v15.9 R2.
Reviewer family: {args.family}
Candidate SHA-256: {sha(cand)}

This is a post-blind challenge of the exact patched synthesis. You have not seen other challenge answers.
...
- At most 6 material findings. Do not invent minor stylistic defects.
...
'''
 ...
 result={'schema':'GardenV159FinalChallengeReview/v1','family':args.family,'model':model,'candidate_sha256':sha(cand),'prompt_sha256':sha(prompt),
