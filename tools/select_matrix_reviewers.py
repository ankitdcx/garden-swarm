...
def main()->int:
    parser=argparse.ArgumentParser(); parser.add_argument("--output",default="agents/runtime/free-selection.json"); parser.add_argument("--slot",type=int); parser.add_argument("--count",type=int,default=3); parser.add_argument("--minimum-count",type=int,default=1); args=parser.parse_args()
    ...
    slot=args.slot if args.slot is not None else int(datetime.now(timezone.utc).timestamp()//3600); models=catalog(key); selected=_adaptive_choose(models,slot,requested=args.count,minimum=args.minimum_count); families=[row["family"] for row in selected]
    ...
    payload={"schema":"GardenFreeModelSelection/v2","purpose":"MULTI_TRACK_REVIEW_MATRIX","hour_slot":slot,"free_catalog_count":len(models),"requested_family_count":args.count,"required_family_count":args.minimum_count,"selected_family_count":len(selected),"selection_scope":"EXECUTION_BUDGET_ONLY_NOT_ADMISSION_QUORUM","selected":selected,"cost_policy":"OpenRouter :free routes only; reviewer calls must independently report usage.cost == 0 before review completion is eligible."}
