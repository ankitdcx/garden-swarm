...
The Garden semantic-admission quorum is enforced downstream, not by this
execution-budget selector. The selector is best-effort above a caller-supplied
minimum so temporary free-model availability does not waste the whole hourly
cycle.
"""
...
def _adaptive_choose(models: list[dict], slot: int, requested: int, minimum: int) -> list[dict]:
    ...
    for count in range(requested, minimum - 1, -1):
        try:
            return choose(models, slot, count=count)
        except SystemExit as exc:
            last_error = exc
    raise SystemExit(
        f"Could not resolve the minimum {minimum} distinct :free reviewer families "
        f"from the live catalog; last_error={last_error}"
    )
...
def main() -> int:
    ...
    parser.add_argument("--count", type=int, default=3, help="preferred/max family count")
    parser.add_argument("--minimum-count", type=int, default=1, help="fail-closed minimum available families")
    ...
    if args.minimum_count < 1 or args.minimum_count > args.count:
        raise SystemExit("minimum-count must be between one and count")
    ...
    models = catalog(key)
    selected = _adaptive_choose(models, slot, requested=args.count, minimum=args.minimum_count)
    ...
    payload = {
        ...
        "purpose": "MULTI_TRACK_REVIEW_MATRIX",
        "hour_slot": slot,
        "free_catalog_count": len(models),
        "requested_family_count": args.count,
        "required_family_count": args.minimum_count,
