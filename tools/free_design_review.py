...
def main() -> int:
    ...
    model = sel["selected"][0]; model_id = model["model"]
    if not model_id.endswith(":free"): raise SystemExit(f"paid route refused: {model_id}")
    slot = int(sel["hour_slot"])
    items = []
    for filename in FILES:
        raw = (root / filename).read_bytes(); text = raw.decode("utf-8"); parts = chunks(text)
...
--- END CANONICAL CHUNK ---"""
    ...
    receipt = {"schema":"GardenFreeDesignReviewReceipt/v1","model":model,"source_file":filename,"source_sha256":file_sha,"chunk_index":index,"chunk_count":total,"usage":usage,"handoff":handoff,"admission_status":"PROPOSALS_ONLY_NEEDS_WHOLE_SOURCE_CROSS_REFERENCE"}
    out = root / args.output; out.parent.mkdir(parents=True, exist_ok=True); out.write_text(json.dumps(receipt, indent=2)+"\n")
    print(json.dumps({"model":model_id,"source":filename,"chunk":f"{index}/{total}","cost":usage.get("cost"),"status":handoff.get("status")}))
    return 0
