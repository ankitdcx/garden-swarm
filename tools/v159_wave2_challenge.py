...
OUT=Path('review-results/v159-wave2-challenge')
MODELS={
...
def prompt(source,candidate,family,ph):
    return f'''SECOND-PASS ADVERSARIAL GARDEN v15.9 REVIEW — WAVE 2.
You are {family}. You now see the original bounded source packet AND ChatGPT's synthesized candidate after blind reviewers were considered. Do not vote by popularity. Attack the candidate.
Preserve: authority/evidence/capability do not manufacture authority; UNKNOWN != PASS; specification != proof != implementation != certification; rights/privacy/safety/law remain hard constraints; prefer existing owners/minimal repair; do not invent universal constants where profiles should own them.
Candidate SHA256: {ph}
...
def main():
    if len(sys.argv)!=3: raise SystemExit('usage: v159_wave2_challenge.py SOURCE CANDIDATE')
    ...
    result={'schema':'GardenWave2Challenge/v1','packet_id':pid,'source_sha256':sha(source),'candidate_sha256':sha(candidate),'completed':completed,'quorum':completed>=3,'reviews':reviews,'canonical_effect':False}
