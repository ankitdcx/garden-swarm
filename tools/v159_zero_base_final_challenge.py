...
SRC=Path('review-inputs/v159-zero-review/ZERO_BASE_SYNTHESIS_R1.txt')
OUT=Path('review-results/v159-zero-review/final-challenge')
DAILY=10.0
PER=0.10
MODELS={
...
def call(model,prompt,key):
 ...
 cmd=['curl','-sS','--connect-timeout','10','--max-time','360',OR,'-X','POST',
      ...
      '-H','X-Title: Garden v15.9 Zero-Base Final Challenge',
      '--data-binary','@-','-w','\\n%{http_code}']
...
Try to falsify the synthesis. Focus on material contradictions, weakened safety/rights/privacy/authority,
...
At most 6 findings. Do not invent stylistic defects.
...
'''
 ...
 res={'schema':'GardenV159ZeroBaseFinalChallenge/v1','family':args.family,'model':model,'candidate_sha256':sha(src),
