...
class ProviderStopTests(unittest.TestCase):
    ...
    def test_paid_batch_stops_after_first_provider_failure(self):
        selection={'schema':'GardenPaidModelSelection/v2','design_epoch':'v15.5','selected':[{'family':f,'model':f+'/m'} for f in ['a','b']], 'approved_families':['a','b'],'routine_hourly_cost_ceiling_usd':0.20,'routine_model_call_cost_ceiling_usd':0.05,'daily_openrouter_cost_ceiling_usd':1}
        with tempfile.TemporaryDirectory() as tmp:
