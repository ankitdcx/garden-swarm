...
class PaidReviewRoutingTests(unittest.TestCase):
    ...
    def test_model_catalog_reference_is_explicit(self): self.assertEqual(self.policy["catalog_reference_date"], "2026-09-15")
    ...
    def _selection(self):
        return {"approved_families":[self.policy["routine_reviewers"][0]["family"]],"max_prompt_characters":10000,"max_output_tokens":100,"routine_model_call_cost_ceiling_usd":0.0125,"provider_policy":self.policy["provider_policy"]}
    ...
    def test_paid_call_transmits_hard_provider_price_ceiling(self):
        ...
        response = {"choices":[{"message":{"content":"{\"ok\": true}"}}],"usage":{"cost":0.001}}
        with patch.dict(os.environ, {"OPENROUTER_API_KEY":"test-key"}), patch("tools.run_paid_matrix_review.request.urlopen", return_value=_Response(response)) as mocked:
    ...
    def test_paid_call_refuses_when_daily_reserved_budget_would_exceed_cap(self):
        selection=self._selection(); selection["daily_openrouter_cost_ceiling_usd"]=1.0; model=self.policy["routine_reviewers"][0]
        with patch.dict(os.environ,{"OPENROUTER_API_KEY":"test-key"}), patch("tools.run_paid_matrix_review._key_usage_daily", return_value=(0.995,{"status":"VERIFIED","usage_daily":0.995})):
            raw, attempt = paid._call(model=model,prompt="bounded public target",selection=selection)
