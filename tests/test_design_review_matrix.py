...
class DesignReviewMatrixTests(unittest.TestCase):
    ...
    def test_bundle_fails_closed_below_independent_threshold(self) -> None:
        ...
        finding = {"reviewer_family": "family-a", "reviewer_model": "example/a:free"}
        bundle = build_bundle(matrix=matrix,target=target,trace=trace,independent=[finding],finals=[],attempts=[{"status":"CALLED","cost":0}])
    ...
    def test_bundle_never_auto_admits_even_after_three_final_dispositions(self) -> None:
        matrix, target = load_matrix(ROOT)
        trace = {"source": target["source_file"], "source_sha256": "0" * 64}
        independent = [{"reviewer_family": family, "reviewer_model": f"example/{family}:free"} for family in ("family-a","family-b","family-c")]
        finals = [dict(row) for row in independent]
        attempts = [{"status":"CALLED","cost":0,"phase":phase,"family":family} for phase in ("INDEPENDENT","PEER_CROSS_EXAMINATION") for family in ("family-a","family-b","family-c")]
    ...
    def test_bundle_refuses_unverified_or_nonzero_cost_as_complete(self) -> None:
        ...
        independent = [{"reviewer_family": family, "reviewer_model": f"example/{family}:free"} for family in ("family-a","family-b","family-c")]
        finals = [dict(row) for row in independent]
        attempts = [{"status":"CALLED","cost":cost} for cost in (0,0,None,0,0,0)]
        bundle = build_bundle(matrix=matrix,target=target,trace=trace,independent=independent,finals=finals,attempts=attempts)
        self.assertEqual(bundle["status"], "COST_VERIFICATION_FAILED")
        self.assertFalse(bundle["zero_cost_verified"])
        self.assertFalse(bundle["semantic_delta_admitted"])

    def test_selector_can_choose_three_distinct_allowed_free_families(self) -> None:
        models = [
            ...
            {"id":"mistralai/blocked:free","context_length":999},
        ]
        ...
        self.assertTrue(all("mistral" not in row["model"].lower() for row in picked))
